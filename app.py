...
# --- 3. Enhanced RAG System (Backend Logic) ---
class RAGSystem:
    ...
    def _expand_query(self, query: str) -> str:
        ...
        for short, full in self.skill_synonyms.items():
            if short in q and full not in q:
                q += f" {full}"
        return q
    ...
    def _parse_and_get_filtered_ids(self, query: str) -> Set[int]:
        ...
        if required_skills:
            candidate_ids.intersection_update({
                emp['id'] for emp in self.employees
                if all(req in [s.lower() for s in emp['skills']] for req in required_skills)
            })
    ...
    def search(self, query: str) -> tuple[List[Employee], np.ndarray]:
        ...
        pre_filtered_ids = self._parse_and_get_filtered_ids(expanded_query)

        if not pre_filtered_ids:
            pre_filtered_ids = set(self.employee_map.keys())
        ...
        for eid in semantic_ids_list[0]:
            if eid != -1 and eid in pre_filtered_ids and eid not in seen:
                final_candidates.append(Employee(**self.employee_map[eid]))
                seen.add(eid)

        return final_candidates, distances
    ...
    def generate_hr_response(self, query: str, context_employees: List[Employee]) -> str:
        system_prompt = """You are an expert HR Talent Acquisition Partner. 
        Based on the user query and retrieved candidate profiles, 
        suggest the best matches, highlight their skills, and justify why they fit."""
...
def show_thinking_animation():
    steps = ["🔍 Parsing query...", "⚙️ Applying filters...", "🧠 Analyzing candidates..."]
...
with st.sidebar:
    ...
    st.markdown("This AI chatbot uses hybrid semantic + filter search to find talent.")
    st.markdown("---")
...
if rag_system:
    ...
    if not st.session_state.messages:
        ...
        for i, p in enumerate(prompts):
            if cols[i].button(p, use_container_width=True, on_click=handle_prompt_click, args=[p]): pass
